 ...
 * SlideUtilities.java
 *
 * Created on May 16, 2006, 10:44 AM
 */
...
public class SlideUtilities {
    ...
    public static void setPermanentSize(Component c, int w, int h) {
    ...
    public static void setDimensions(Window window, int x, int y, int w, int h) {
    ...
    private static int oldClickCountToStart = 2;
    
    public static void enableSingleClickCellEditing(javax.swing.JTable t) {
        javax.swing.DefaultCellEditor editor =
            (javax.swing.DefaultCellEditor)t.getDefaultEditor(String.class);
        
        oldClickCountToStart = editor.getClickCountToStart();
        editor.setClickCountToStart(1);
    }
    
    public static void disableSingleClickCellEditing(javax.swing.JTable t) {
        javax.swing.DefaultCellEditor editor =
            (javax.swing.DefaultCellEditor)t.getDefaultEditor(String.class);
        
        editor.setClickCountToStart(oldClickCountToStart);
    }
    
    private static MouseAdapter easyCellEditorReleasingAdapter = null;
    
    public static void enableEasyCellEditorReleasing(final javax.swing.JTable t) {
        easyCellEditorReleasingAdapter = new MouseAdapter() {
            public void mousePressed(MouseEvent me) {
                java.awt.Point p = me.getPoint();
                
                if (t.rowAtPoint(p) < 0 || t.columnAtPoint(p) < 0) {
                    javax.swing.CellEditor editor = t.getCellEditor();
        ...
        t.addMouseListener(easyCellEditorReleasingAdapter);
    }
    
    public static void disableEasyCellEditorReleasing(javax.swing.JTable t) {
        t.removeMouseListener(easyCellEditorReleasingAdapter);
    }
    
    private static boolean forcingTableEdit = false;
    
    public static boolean isForcingTableEdit() {
        return forcingTableEdit;
    }

    public static Component forceEdit(javax.swing.JTable t, MouseEvent me) {
        java.awt.Point p = me.getPoint();
        
        p = new java.awt.Point(p.x, p.y-52);
        ...
        javax.swing.JComponent c = (javax.swing.JComponent) t.getEditorComponent();
        
        return c;
    }
    
    public static List<String> getMenuNames(javax.swing.JMenuBar menuBar) {
    ...
    public static List<String> getMenuItemNames(javax.swing.JMenu menu) {
        List<String> menuItemNames = new ArrayList();
        
        Component[] menuItemComponents = menu.getMenuComponents();
        
        for (Component c : menuItemComponents) {
    ...
    public static JMenu getMenu(javax.swing.JMenuBar menuBar, String menuName) {
        int menuIndex = getIndexOfMenuByName(menuBar, menuName);
        
        if (menuIndex < 0) {
            return null;
        }
        
        return (JMenu) menuBar.getMenu(menuIndex);
    }
    
    public static int getIndexOfMenuByName(javax.swing.JMenuBar menuBar, String menuName) {
        List<String> menuNames = getMenuNames(menuBar);
        
        for (int i = 0; i < menuNames.size(); i++) {
            String s = menuNames.get(i);
            
            if (s != null && s.equals(menuName)) {
                return i;
            }
        }
    ...
    public static int getIndexOfMenuItemByName(JMenu menu, String menuItemName) {
        return getMenuItemNames(menu).indexOf(menuItemName);
    }
    ...
    public static JSplitPane createJSplitPane(int mode, Component c1, Component c2) {
        Color oldColor = UIManager.getColor("SplitPane.background");
        Border oldBorder = UIManager.getBorder("SplitPane.border");
        UIManager.put("SplitPane.background", Color.WHITE);
        UIManager.put("SplitPane.border", BorderFactory.createEmptyBorder());
        
        JSplitPane splitPane = new JSplitPane(mode, c1, c2);
        
        UIManager.put("SplitPane.background", oldColor);
        UIManager.put("SplitPane.border", oldBorder);
    ...
    public static void disableBuiltInFocusManagement(javax.swing.JComponent c) {
        javax.swing.LookAndFeel.installProperty(c,
            "focusTraversalKeysForward", new java.util.TreeSet());
        
        javax.swing.LookAndFeel.installProperty(c,
            "focusTraversalKeysBackward", new java.util.TreeSet());
    }

    public static boolean isListening(Object[] l, EventListener el) {
        return java.util.Arrays.asList(l).contains(el);
    }

    public static boolean isListening(List l, EventListener el) {
        return isListening(l.toArray(), el);
    }

    public static boolean isListening(JComponent c, Class clazz, EventListener el) {
        return isListening(c.getListeners(clazz), el);
    }

    public static URL getResource(String className, String name) {
        Class mainClass = org.rummage.toolbox.util.Properties.getMainClass();
        
        if (mainClass == null) {
            System.out.println("Couldn't find presumed main class: " + className);
    ...
    public static void center(JComponent c, JPanel panel) {
        c.setLocation((panel.getWidth() - c.getWidth()) / 2, 
            (panel.getHeight() - c.getHeight()) / 2);
    }
}
